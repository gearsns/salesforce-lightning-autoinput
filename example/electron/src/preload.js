...
class SalesforceLightningAutoinput {
	...
	inputValue = async (el_actionbody, label_name, input_value, section_name) => {
		let cur_section_name = "";
		let ret = await this.FindAllDescendants(el_actionbody, async node => {
			if (!node || !node.tagName || !node.tagName.match) {
				return null;
			}
			if (node.className && node.className.match && node.className.match(/(^| +)section\-header\-title($| +)/)) {
		...
		if (!ret) {
			ret = await this.FindAllDescendants(el_actionbody, async node => {
				if (!node || !node.tagName || !node.tagName.match) {
					return null;
				}
				if (node.tagName === "LABEL" && node.innerText === label_name) {
					...
					if (el_input.className.match(/(^| +)uiInputTextForAutocomplete($| +)/)) {
						...
						for (let i = 0; i < 50; i++) {
							...
							el_input.dispatchEvent(new KeyboardEvent("keydown", { key: "a" }));
							el_input.click();
							let cc_node = await this.FindAllDescendants(node.parentNode, async cc_node => {
								if (cc_node && cc_node.className && cc_node.className.match
									&& cc_node.className.match(/(^| +)slds\-lookup__result\-text($| +)/)
									&& cc_node.innerText && cc_node.innerText.split(/ /).sort().join(" ").toUpperCase() === check_value) {
									return cc_node;
								}
								return null;
	...
	run = async (key, value, section) => {
		...
		if (key.type === "anchor") {
			...
			if (el_actionbody) {
				await this.anchorClick(el_actionbody, value, key.wait);
			}
			...
			let el_actionbody = await this.getActionBodyElement();
			if (el_actionbody) {
				await this.buttonWait(el_actionbody, value);
			}
		} else {
			for (let i = 0; i < 50; ++i) {
				if (this.cancellationToken && this.cancellationToken.isCancellationRequested) {
